...
func (cq *CheckQuery) QueryWebsite() *WebsiteQuery {
...
func (cq *CheckQuery) Clone() *CheckQuery {
	...
	return &CheckQuery{
		...
		withWebsite: cq.withWebsite.Clone(),
...
func (cq *CheckQuery) WithWebsite(opts ...func(*WebsiteQuery)) *CheckQuery {
	query := (&WebsiteClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withWebsite = query
	return cq
}
...
func (cq *CheckQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Check, error) {
	...
	if query := cq.withWebsite; query != nil {
		if err := cq.loadWebsite(ctx, query, nodes, nil,
			func(n *Check, e *Website) { n.Edges.Website = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CheckQuery) loadWebsite(ctx context.Context, query *WebsiteQuery, nodes []*Check, init func(*Check), assign func(*Check, *Website)) error {
...
func (cq *CheckQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withWebsite != nil {
			_spec.Node.AddColumnOnce(check.FieldWebsiteID)
		}
	}
